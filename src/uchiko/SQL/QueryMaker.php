...
namespace {
    ...
    function sql_op() {
        ...
        list($num_args, $builder) = uchiko\SQL\QueryMaker::_compileBuilder($expr);
    ...
    function sql_raw($sql, $bind) {
        return uchiko\SQL\QueryMaker::_new(null, function() use ($sql) {
            return $sql;
        }, $bind);
    }
    ...
    function _call_andor($fn, $fn_args) {
        ...
        return \uchiko\SQL\QueryMaker::_new($column, function($column, $quote_cb) use ($args, $fn, $op) {
            if ( count($args) === 0 ) {
    ...
    function _call_fnop($fn, $fn_args) {
        ...
        list($num_args, $builder) = \uchiko\SQL\QueryMaker::_compileBuilder($FNOP[$op]);
    ...
    function _sql_op($fn, $builder, $column, $args) {
        return \uchiko\SQL\QueryMaker::_new($column, function($column, $quote_cb) use ($builder) {
            if ( is_null($column) ) {
